...
window.sfun = (function($, undefined) {
  ...
  this['hashUpdate'] = function(options, push, numbListener) {
    ...
    if (numbListener) {
      this['hashNumbQueue'].push(hash);
      if (debug) {
        console.log('adding hash('+hash+') on to numbQueue, now len['+this['hashNumbQueue'].length+']');
      }
    }
  ...
  this['handler_hashChanged'] = function(hash) {
    ...
    var index = this['hashNumbQueue'].indexOf(hash);
    if (index != -1) {
      if (debug) {
        console.log('ignoring hash('+hash+') change');
      }
      // remove this hash from the numbQueue
      this['hashNumbQueue'].splice(index, 1);
    }
  ...
  this['handler_mouseWheeled'] = function(event) {
    ...
    if (direction == 'x') {
      ...
      $(document).scrollLeft(xpos + (0 - event.deltaY) * cellsize);
      if (debug && false) {
  ...
  this['setVisibleNewlyVisible'] = function(scrolldir) {
    ...
    var jqImg = $('ul.flow li.cell img.visible:'+(scrolldir > 0 ? 'last' : 'first'));
  ...
  this['setVisibleNewlyHidden'] = function(scrolldir) {
    ...
    var jqImg = $('ul.flow li.cell img.visible:'+(scrolldir > 0 ? 'first' : 'last'));
    // initial sequence number is the one after the last visible, or before the first visible
    ...
    jqImg = $('ul.flow li.cell img.selected');
    if (!jqImg.hasClass('visible')) {
      if (debug) {
        console.log('previously selected image '+seq+' no longer visible');
      }
      // find the next visible one in the scroll direction
      jqImg = $('ul.flow li.cell img.visible:'+(scrolldir > 0 ? 'first' : 'last'));
